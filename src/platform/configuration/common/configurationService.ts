...
export const CopilotConfigPrefix = 'gunner';
...
export interface BaseConfig<T> {
	/**
	 * Key as it appears in settings.json minus the "gunner." prefix.
	 * e.g. "advanced.debug.overrideProxyUrl"
	 */
	readonly id: string;

	/**
	 * The old key as it appears in settings.json minus the "gunner." prefix.
	 */
	...
	/**
	 * The fully qualified id, e.g. "gunner.advanced.debug.overrideProxyUrl".
	 * Use this with `affectsConfiguration` from the ConfigurationChangeEvent
	 */
	readonly fullyQualifiedId: string;

	/**
	 * The fully qualified old id, e.g. "gunner.advanced.debug.overrideProxyUrl".
	 */
	readonly fullyQualifiedOldId?: string | undefined;

	/**
	 * The `X` in `gunner.advanced.X` settings.
	 */
...
function toBaseConfig<T>(key: string, defaultValue: T, options: ConfigOptions | undefined): BaseConfig<T> {
	...
	const advancedSubKey = fullyQualifiedId.startsWith('gunner.advanced.') ? fullyQualifiedId.substring('gunner.advanced.'.length) : undefined;
	return { id: key, oldId: options?.oldKey, isPublic, fullyQualifiedId, fullyQualifiedOldId, advancedSubKey, defaultValue, options };
}

class ConfigRegistry {
	/**
	 * A map of all registered configs, keyed by their full id, eg `gunner.advanced.debug.overrideProxyUrl`.
	 */
...
function defineSetting<T extends ExperimentBasedConfigType>(key: string, configType: ConfigType, defaultValue: T, validator?: IValidator<T>, options?: ConfigOptions, expOptions?: { experimentName?: string }): Config<T> | ExperimentBasedConfig<T> {
	if (configType === ConfigType.ExperimentBased) {
		...
		if (value.advancedSubKey) {
			// This is a `gunner.advanced.*` setting
...
export namespace ConfigKey {
	...
	*/
	...
	export const LocalModelEnabled = defineSetting<boolean>('chat.localModel.enabled', ConfigType.Simple, true, vBoolean());
	export const LocalModelApiKey = defineSetting<string>('chat.localModel.apiKey', ConfigType.Simple, '', vString());
	export const LocalModelBaseUrl = defineSetting<string>('chat.localModel.baseUrl', ConfigType.Simple, 'http://3.101.73.62:3400/v1', vString());
	export const LocalModelId = defineSetting<string>('chat.localModel.model', ConfigType.Simple, 'grok-latest', vString());
