...
const reviewDiffContextKey = 'gunner.chat.reviewDiff.enabled';
const reviewDiffReposContextKey = 'gunner.chat.reviewDiff.enabledRootUris';
const numberOfReviewCommentsKey = 'gunner.chat.review.numberOfComments';
...
export class ReviewServiceImpl implements IReviewService {
	...
	private _commentController = vscode.comments.createCommentController('gunner-review', 'Code Review');
	...
	constructor(
		...
		this._disposables.add(vscode.workspace.onDidChangeConfiguration(e => {
			...
			if (e.affectsConfiguration('gunner.advanced') || e.affectsConfiguration('gunner.advanced.review.intent')) {
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = vscode.languages.createDiagnosticCollection('gunner.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (this._comments.length === 1) {
				vscode.commands.executeCommand('gunner.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					...
					if (active !== this._activeThread) {
						...
						if (active) {
							vscode.commands.executeCommand('gunner.chat.review.current', active);
						}
					}
				}, 500);
			}
