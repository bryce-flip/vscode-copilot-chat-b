...
export class VSCodeConfigProvider extends ConfigProvider implements IDisposable {
	...
	override onDidChangeCopilotSettings: ConfigProvider['onDidChangeCopilotSettings'] = transformEvent(
		...
		event => {
			if (event.affectsConfiguration('gunner')) {
				return this;
			}
			if (event.affectsConfiguration('gunner')) {
				return this;
...
export class VSCodeEditorInfo implements ICompletionsEditorAndPluginInfo {
	...
	getEditorPluginInfo() {
		return { name: 'gunner', readableName: 'Gunner for Visual Studio Code', version: packageJson.version };
	}
...
export async function enableCompletions(accessor: ServicesAccessor) {
	...
	// The rest of this function is the inverse of disableCompletions(), updating the gunner.enable setting.
...
/**
 * Disable completions using the gunner.enable setting.
 */
