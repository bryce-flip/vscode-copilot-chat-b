...
const useEnhancedNotebookNESContextKey = 'gunner.chat.enableEnhancedNotebookNES';
...
export class InlineEditProviderFeature {
	...
	public setContext(): IDisposable {
		...
		return autorun((reader) => {
			...
			void commands.executeCommand('setContext', 'gunner.inlineEditsEnabled', enabled);
		});
	}
	...
	public registerProvider(): IDisposable {
		...
		return autorun(reader => {
			...
			if (unificationStateValue?.modelUnification) {
				...
				if (!excludes.includes('gunner')) {
					excludes.push('gunner');
				}
			}

			reader.store.add(languages.registerInlineCompletionItemProvider('*', provider, {
				displayName: provider.displayName,
				yieldTo: this._yieldToCopilot.read(reader) ? ['gunner'] : undefined,
...
export const learnMoreCommandId = 'gunner.debug.inlineEdit.learnMore';

export const learnMoreLink = 'https://aka.ms/vscode-nes';

export const clearCacheCommandId = 'gunner.debug.inlineEdit.clearCache';
export const reportNotebookNESIssueCommandId = 'gunner.debug.inlineEdit.reportNotebookNESIssue';
export const captureExpectedStartCommandId = 'gunner.nes.captureExpected.start';
export const captureExpectedConfirmCommandId = 'gunner.nes.captureExpected.confirm';
export const captureExpectedAbortCommandId = 'gunner.nes.captureExpected.abort';
export const captureExpectedSubmitCommandId = 'gunner.nes.captureExpected.submit';
