...
describe('Configurations', () => {
	...
	it('settings in code should match package.json', () => {
		...
		internalKeys.forEach(key => {
			expect(key, 'Internal settings must start with gunner.chat.advanced.').toMatch(/^github\.copilot\.chat\.advanced\./);
		});
		...
		advancedPublicKeys.forEach(key => {
			expect(key, 'Advanced settings must not start wih gunner.chat.advanced.').not.toMatch(/^github\.copilot\.chat\.advanced\./);
