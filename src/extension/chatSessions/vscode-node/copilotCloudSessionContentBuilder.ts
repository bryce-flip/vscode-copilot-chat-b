		...
		const sessionResults = await Promise.all(
			sessions.map(async (session, sessionIndex) => {
				...
				if (sessionIndex === 0 && pullRequest.author && vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
					...
					const card = new vscode.ChatResponsePullRequestPart({ command: 'gunner.chat.openPullRequestReroute', title: vscode.l10n.t('View Pull Request {0}', `#${pullRequest.number}`), arguments: [pullRequest.number] }, pullRequest.title, plaintextBody, getAuthorDisplayName(pullRequest.author), `#${pullRequest.number}`);
