...
const LAST_USED_ISOLATION_OPTION_KEY = 'gunner.cli.lastUsedIsolationOption';
const OPEN_REPOSITORY_COMMAND_ID = 'gunner.cli.sessions.openRepository';
const OPEN_IN_COPILOT_CLI_COMMAND_ID = 'gunner.cli.openInCopilotCLI';
...
namespace SessionIdForCLI {
	export function getResource(sessionId: string): vscode.Uri {
		return vscode.Uri.from({
			scheme: 'gunnercli', path: `/${sessionId}`,
		});
	}
	...
	export function isCLIResource(resource: vscode.Uri): boolean {
		return resource.scheme === 'gunnercli';
	}
}
...
export class CopilotCLIChatSessionContentProvider extends Disposable implements vscode.ChatSessionContentProvider, ICopilotCLIChatSessionItemProvider {
		...
		const controller = this.controller = this._register(vscode.chat.createChatSessionItemController(
			...
			async () => {
				...
				try {
					...
					void this.commandExecutionService.executeCommand('setContext', 'gunner.chat.cliSessionsEmpty', count === 0);
	...
	public async provideChatSessionItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionItem[]> {
		...
		this.commandExecutionService.executeCommand('setContext', 'gunner.chat.cliSessionsEmpty', count === 0);
...
export class CopilotCLIChatSessionParticipant extends Disposable {
	...
	private async handleDelegationFromAnotherChat(
		...
		try {
			...
			await vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.gunnercli', {
...
export function registerCLIChatCommands(
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.delete', async (sessionItem?: vscode.ChatSessionItem) => {
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.resumeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			const id = SessionIdForCLI.parse(sessionItem.resource);
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		const id = SessionIdForCLI.parse(sessionItem.resource);
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.setTitle', async (sessionItem?: vscode.ChatSessionItem, title?: string) => {
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.newSession', async () => {
		await createCopilotCLITerminal('editor', l10n.t('Copilot CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.newSessionToSide', async () => {
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.openWorktreeInNewWindow', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.openWorktreeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.copyWorktreeBranchName', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.chat.applyCopilotCLIAgentSessionChanges', applyChanges));
	disposableStore.add(vscode.commands.registerCommand('gunner.chat.applyCopilotCLIAgentSessionChanges.apply', applyChanges));
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.chat.mergeCopilotCLIAgentSessionChanges.merge', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource);
	}));

	disposableStore.add(vscode.commands.registerCommand('gunner.chat.mergeCopilotCLIAgentSessionChanges.mergeAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource, true);
	}));

	disposableStore.add(vscode.commands.registerCommand('gunner.chat.updateCopilotCLIAgentSessionChanges.update', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.sessions.refreshChanges', async (resource?: vscode.Uri) => {
		if (!resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.sessions.initializeRepository', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.sessions.commit', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.sessions.commitAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.sessions.sync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.sessions.discardChanges', async (sessionResource: vscode.Uri, ref: string, ...resources: vscode.Uri[]) => {
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.chat.createPullRequestCopilotCLIAgentSession.createPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.chat.createDraftPullRequestCopilotCLIAgentSession.createDraftPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.chat.createPullRequestCopilotCLIAgentSession.updatePR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.chat.openPullRequestCopilotCLIAgentSession.openPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.commitToWorktree', async (args?: { worktreeUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
	...
	disposableStore.add(vscode.commands.registerCommand('gunner.cli.sessions.commitToRepository', async (args?: { repositoryUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
