...
export class RemoteAgentContribution implements IDisposable {
	...
	private registerAgent(agentData: IAgent | null): Disposable {
		...
		const participantId = `gunner-dynamic.${agentData?.slug ?? GITHUB_PLATFORM_AGENT_ID}`;
			...
			if (!agentData?.slug) {
				...
				if (!accessToken) {
					if (this.authenticationService.isMinimalMode) {
						responseStream.markdown(l10n.t('Minimal mode is enabled. You will need to change `gunner.advanced.authPermissions` to `default` to use this feature.'));
						responseStream.button({
							title: l10n.t('Open Settings (JSON)'),
							command: 'workbench.action.openSettingsJson',
							arguments: [{ revealSetting: { key: 'gunner.advanced.authPermissions' } }]
						});
					} else {
