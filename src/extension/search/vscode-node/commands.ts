...
export class SearchPanelCommands extends Disposable {
	constructor(
		...
		super();
		this._register(vscode.commands.registerCommand('gunner.search.markHelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Helpful);
		}));
		this._register(vscode.commands.registerCommand('gunner.search.markUnhelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Unhelpful);
		}));
		this._register(vscode.commands.registerCommand('gunner.search.feedback', () => {
			this.sendFeedback(SearchFeedbackKind.Feedback);
			vscode.commands.executeCommand('gunner.report', `Copilot search feedback: "${SemanticSearchTextSearchProvider.latestQuery}"`);
		}));
	}
