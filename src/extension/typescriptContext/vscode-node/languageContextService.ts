...
const showContextInspectorViewContextKey = `gunner.chat.showContextInspectorView`;
export class InlineCompletionContribution implements vscode.Disposable, TokenBudgetProvider {
	...
	constructor(
		...
		if (languageContextService instanceof LanguageContextServiceImpl) {
			this.disposables.add(vscode.commands.registerCommand('gunner.debug.showContextInspectorView', async () => {
				await vscode.commands.executeCommand('setContext', showContextInspectorViewContextKey, true);
				await vscode.commands.executeCommand('gunner-context-inspector.focus');
			}));
			this.disposables.add(vscode.window.registerTreeDataProvider('gunner-context-inspector', new InspectorDataProvider(languageContextService)));
		}
