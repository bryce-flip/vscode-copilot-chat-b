...
export class DebugCommandsContribution extends Disposable {
	constructor(
		...
		this._register(vscode.commands.registerCommand('gunner.debug.generateSTest', async () => {
		...
		this._register(vscode.commands.registerCommand('gunner.createLaunchJsonFileWithContents', async (launchConfig: IStartDebuggingParsedResponse) => {
		...
		this._register((vscode.commands.registerCommand('gunner.startDebugging', async (config: IStartDebuggingParsedResponse, progress) => {
